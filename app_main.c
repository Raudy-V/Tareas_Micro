...
#define FTC 8 // Fotocelda
#define Run_Time 1200 // Tiempo de RT; 2 minutos (Tiempo en ticks)
#define Motor_ON 1
#define Motor_OFF 0
#define Lamp_ON 1
#define Lamp_OFF 0
...
struct IO {
    unsigned int fca; // Final de carrera arriba
    unsigned int fcc; // Final de carrera abajo
    unsigned int ftc; // Fotocelda
    unsigned int bc; // Botón cerrar
    unsigned int ba; // Botón abrir
    unsigned int bs; // Botón stop
    unsigned int mc; // Motor cerrar
    unsigned int ma; // Motor abrir
    unsigned int lamp;  // Luz
    unsigned int buzzer; // Bocina
...
unsigned char movimiento_activo = 0;
...
int Func_Estado_Error (void)
{
    ...
    if (io.bs) {  // Para forzar el estado a inicio(Reset basicamente)
